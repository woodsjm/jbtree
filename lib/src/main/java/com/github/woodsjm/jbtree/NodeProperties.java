...
class NodeProperties<T extends Comparable<T>> {
  ...
  NodeProperties(
    ...
    if (minNodeValue instanceof Integer
        || minNodeValue instanceof Float
        || minNodeValue instanceof String) {
      this.minNodeValue = minNodeValue;
    }

    if (maxNodeValue instanceof Integer
        || maxNodeValue instanceof Float
        || maxNodeValue instanceof String) {
  ...
  final boolean symmetricHelper(Node<T> left, Node<T> right) {
    ...
    return areValsEqual
        && symmetricHelper(lLeftChild, rRightChild)
        && symmetricHelper(lRightChild, rLeftChild);
  }
}
