    ...
    while (currentNodes.size() > 0) {
      ...
      for (Node<T> node : currentNodes) {
        ...
        if (node.getLeft() != null && node.getLeft().getVal() != null && val != null) {
          // FIX: Check for mismatched NodeValue types (e.g. order comparision of letters and
          // numbers)
        ...
        try {
          if (nodesSeen.contains(node)) {
            ...
            if (node.val instanceof Float) {
              msg =
                  String.format(
                      "cyclic reference at Node(%.2f) (level-order index %s)", node.val, nodeIdx);
            } else {
              msg =
                  String.format(
                      "cyclic reference at Node(%s) (level-order index %s)", node.val, nodeIdx);
            }
          ...
          if (!(node.val instanceof Integer
              || node.val instanceof String
              || node.val instanceof Float)) {
