...
class BtreeCharacterBox {
  ...
  static <T extends Comparable<T>> BtreeCharacterBox buildTreeString(
      Node<T> root, int currentIdx, boolean includeIdx, String delimiter) {
    if (root == null) {
    ...
    nodeRepr =
        includeIdx
            ? String.format("%s%s%s", currentIdx, delimiter, root.getVal())
    ...
    BtreeCharacterBox l =
        buildTreeString(root.getLeft(), 2 * currentIdx + 1, includeIdx, delimiter);
    BtreeCharacterBox r =
        buildTreeString(root.getRight(), 2 * currentIdx + 2, includeIdx, delimiter);
    ...
    for (int i = 0;
        i < Math.max(l.box == null ? 0 : l.box.size(), r.box == null ? 0 : r.box.size());
        i++) {
      String lLine = "";
      String rLine = "";
      if (l.box != null) {
        lLine =
            i < l.box.size() ? l.box.get(i) : String.valueOf(new char[l.width]).replace("\0", " ");
      }
      if (r.box != null) {
        rLine =
            i < r.box.size() ? r.box.get(i) : String.valueOf(new char[r.width]).replace("\0", " ");
      }
